...
(** On the other hand, in type theory, equality is a very contentious subject. *)
(** There are at least three different notions of equality that are important in Coq, *)
(** and researchers are actively investigating new definitions of what it means for two terms to be equal. *)
(** Even once we fix a notion of equality, there are inevitably tricky issues that arise in *)
(** proving properties of programs that manipulate equality proofs explicitly. *)
(** In this chapter, I will focus on design patterns for circumventing these *)
(** tricky issues, and I will introduce the different notions of equality as they are *)
(** germane. *)


(** * The Definitional Equality *)

(** We have seen many examples so far where proof goals follow "by computation." *)
(** That is, we apply computational reduction rules to reduce the goal to a *)
(** normal form, at which point it follows trivially. *)
(** Exactly when this works and when it does not depends on the *)
(** details of Coq's%\index{definitional equality}% _definitional equality_. *)
(** This is an untyped binary relation appearing in the formal metatheory of CIC. *)
(** CIC contains a typing rule allowing the conclusion [E : T] *)
(** from the premise [E : T'] and a proof that [T] and [T'] are definitionally equal.
